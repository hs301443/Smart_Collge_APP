...
export const setupChatSockets = (io: Server) => {
  ...
  io.on("connection", (socket: AuthSocket) => {
    ...
    // Send Message
    // ==========================
    socket.on("send-message", async (data: { roomId: string; content?: string; attachment?: { type: string; url: string } }) => {
      ...
      const message = await MessageModel.create({
        ...
        deliveredTo: room ? room.participants.map((p: any) => p.user.toString()) : [data.roomId],
      });
    ...
    socket.on("seen-message", async ({ roomId, messageId }) => {
      ...
      if (!message.seenBy.some((u: any) => u.toString() === socket.userId)) {
    ...
    socket.on("add-to-group", async (data: { roomId: string; userId: string; role: "User" | "Admin" }) => {
      if (socket.role !== "Admin") return;
      const room = await RoomModel.findById(data.roomId);
      if (!room) return socket.emit("error", { message: "Room not found" });

      if (!room.participants.some((p: any) => p.user.toString() === data.userId)) {
    ...
    socket.on("remove-from-group", async (data: { roomId: string; userId: string }) => {
      ...
      room.participants = room.participants.filter((p: any) => p.user.toString() !== data.userId);
      await room.save();
